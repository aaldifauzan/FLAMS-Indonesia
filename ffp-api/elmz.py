...
class elm():
    ...
    '''
    def __init__(self, hidden_units, activation_function, x, y, C, random_type):
        ...
        if self.random_type == 'he':
            self.W = np.random.randn(self.hidden_units, self.x.shape[1]) * np.sqrt(2 / self.x.shape[1])
